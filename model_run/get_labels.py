...
#Common Parameters
#data ="data/yikers.csv" # The file in csv format which contains the topic terms that needs a label.
#data ="data/bertopic.csv"
model_name ="bertopic_all-roberta-large-v1_np_mini_stopwords"
#model_name = "top2vec_doc2vec_np_mini_stopwords.csv"
#model_name = "lda_embedding_p1000_t30_np_mini_stopwords.csv"
data =os.path.join("data",model_name+".csv")
...
output_filename = f"output_candidates_${model_name}"  # Name of the output file that will store the candidate labels.
...
cand_gen_output = f"output_candidates_${model_name}" # The file which contains candiate generation output.Also used to get supervised output
out_unsup = f"output_unsup_${model_name}" # The Output File name for unsupervised labels
...
out_sup =f"output_supervised_${model_name}" # The output file for supervised labels.
